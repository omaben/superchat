    ...
    <div style="overflow: auto;height: 100%;">
        <ul class="member-in-group">
            <li v-for="item in chatMembersSorting" :key="item.id" @click="detailInfo(item)">
                ...
                <p style="margin-right:5px">{{item.id === userInfo.id ? 'You' : item.nikeName}} <span :class="{'green':item.role===2}">{{item.role | filterRole}}</span></p>
                <img class="el-icon-delete-solid" :src="require('@/assets/svg/trash-ico.svg')" @click.stop="del(item)" v-if="(info.groupRole === 3 || info.groupRole === 2) && item.id!==userInfo.id && item.role !=3 ">
            </li>
        </ul>
        <div v-if="info.hideMemberList !=1 ||  info.groupRole === 3 || info.groupRole === 2">
            <el-collapse-transition>
                <ul class="member-in-group" v-if="viewAllMembers">
        ...
        <el-button class="member-view-all" @click="getAllMembers()" v-if="info.hideMemberList !=1 || info.groupRole === 3 || info.groupRole === 2">{{!viewAllMembers?$t('view_all'):$t('hide_all')}}</el-button>
...
export default {
    ...
    computed:{
        ...
        normalMembersSorting: function(){
            if(this.normalMembers){
                if(this.normalMembers.length>0){
                        this.normalMembers.sort((p1,p2) => {
    ...
    methods: {
         ...
         async detailInfo(item){
             ...
             let chatRoom2 = {}
                ...
                if(userInfo){
                    ...
                    userInfo.avatar = userInfo.nikeName
                    chatRoom2 = {...userInfo, roomType:1,isFromGroup:3}
        ...
        async getAllMembers(){
            ...
            this.viewAllMembers = !this.viewAllMembers
        }
