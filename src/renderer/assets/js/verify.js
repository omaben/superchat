...
import { captchaPuzzleVer } from "@/services/index";
...
const lang = require(`@/i18n/module/common/${key}.js`);
...
(function($, window, document, undefined) {
    ...
    var Slide = function(ele, opt) {
        ...
        (this.defaults = {
            ...
            vOffset: 1,
            vSpace: 5,
            explain: common.slide.slide_text,
            imgUrl: "",
            smallImg: "",
            states: false,
            imgSize: {
    ...
    Slide.prototype = {
        init: function() {
            ...
            //close
            _this.$element.find(".verify-close").on("click", function() {
        ...
        //初始化加载
        loadDom: function() {
            // this.img_rand = Math.floor(Math.random() * 60);			//随机的背景图片
            ...
            if (this.options.type != 1) {
                ...
                panelHtml +=
                    '<div class="verify-img-out"><div class="verify-img-panel"><div class="verify-close" style="z-index:3"><i class="iconfont icon-close"></i></div><div class="verify-refresh" style="z-index:3"><i class="iconfont icon-refresh"></i></div><canvas  class="verify-img-canvas" width="' +
            ...
            panelHtml +=
                tmpHtml +
                '<div class="verify-bar-area"><div class="verify-msg">' +
                this.options.explain +
                '</div><div class="verify-left-bar"><div  class="verify-move-block"><i  class="verify-icon iconfont icon-right"></i></div></div></div>';
            ...
            this.htmlDoms = {
                ...
                close: this.$element.find(".verify-close"),
            };
            ...
            this.htmlDoms.bar_area.css({
                width: parseInt(this.setSize.bar_width) - 10 + "px",
                height: parseInt(this.setSize.bar_height) - 20 + "px",
                "line-height": parseInt(this.setSize.bar_height) - 20 + "px",
                padding: "10px 5px",
            });
        ...
        start: function(e) {
            if (this.isEnd == false) {
                this.htmlDoms.msg.text(common.slide.slide_textend);
                this.htmlDoms.move_block.css("background-color", "#18A452");
                this.htmlDoms.left_bar.css("background-color", "#18A452");
        ...
        move: function(e) {
            if (this.status && this.isEnd == false) {
                ...
                var bar_area_left = Slide.prototype.getLeft(
                    this.htmlDoms.bar_area[0]
                );
                ...
                if (this.options.type != 1) {
                    ...
                    if (
                        ...
                        this.$element
                            .find(".verify-msg:eq(1)")
                            .text(common.slide.slide_textend);
                        move_block_left =
                            this.htmlDoms.bar_area[0].offsetWidth -
                            parseInt(parseInt(this.setSize.bar_height) / 2) +
                            10;
                ...
                if (
                    ...
                    move_block_left = parseInt(
                        parseInt(this.setSize.block_width) / 2
                    );
                }
                //拖动后小方块的left值
                this.htmlDoms.move_block.css(
        ...
        //鼠标松开
            ...
            if (this.status && this.isEnd == false) {
                var postData = {
                    X: parseInt(this.htmlDoms.sub_block.css("left")) - 10,
                    CodeType: _this.options.vOffset,
                    codeId: _this.options.codeId,
                    // account: $("#username").val()
                };
                captchaPuzzleVer(postData).then((res) => {
                    let datas = res;
                    if (datas.success) {
                        ...
                        _this.isEnd = true;
                        _this.options.success(_this, datas);
                    } else {
                        ...
                        _this.options.error(_this, datas);
                    }
                });
                this.status = false;
            }
        ...
        resetSize: function(obj) {
            ...
            var parentWidth =
                obj.$element.parent().width() || $(window).width();
            var parentHeight =
                obj.$element.parent().height() || $(window).height();

            if (obj.options.imgSize.width.indexOf("%") != -1) {
                img_width =
                    (parseInt(obj.options.imgSize.width) / 100) * parentWidth +
                    "px";
            } else {
            ...
            if (obj.options.imgSize.height.indexOf("%") != -1) {
                img_height =
                    (parseInt(obj.options.imgSize.height) / 100) *
                    parentHeight +
                    "px";
            } else {
            ...
            if (obj.options.barSize.width.indexOf("%") != -1) {
                bar_width =
                    (parseInt(obj.options.barSize.width) / 100) * parentWidth +
                    "px";
            } else {
            ...
            if (obj.options.barSize.height.indexOf("%") != -1) {
                bar_height =
                    (parseInt(obj.options.barSize.height) / 100) *
                    parentHeight +
                    "px";
            } else {
            ...
            if (obj.options.blockSize) {
                if (obj.options.blockSize.width.indexOf("%") != -1) {
                    block_width =
                        (parseInt(obj.options.blockSize.width) / 100) *
                        parentWidth +
                        "px";
                } else {
                ...
                if (obj.options.blockSize.height.indexOf("%") != -1) {
                    block_height =
                        (parseInt(obj.options.blockSize.height) / 100) *
                        parentHeight +
                        "px";
                } else {
            ...
            if (obj.options.circleRadius) {
                if (obj.options.circleRadius.indexOf("%") != -1) {
                    circle_radius =
                        (parseInt(obj.options.circleRadius) / 100) *
                        parentHeight +
                        "px";
                } else {
        ...
        randSet: function() {
            var top =
                (this.options.rand1 * parseInt(this.setSize.img_height)) / 15 +
                parseInt(this.setSize.img_height) * 0.1;
            var left =
                (this.options.rand2 * parseInt(this.setSize.img_width)) / 15 +
            ...
            this.htmlDoms.sub_block.css({
                top: parseInt(this.options.rand2) + 10 + "px",
            });
        },
        ...
        refresh: function() {
            ...
            this.htmlDoms.left_bar.animate({ width: parseInt(this.setSize.bar_height) },
                "fast"
            );
            ...
            this.isEnd = false;
            this.htmlDoms.sub_block.css("left", "10px");
        },
